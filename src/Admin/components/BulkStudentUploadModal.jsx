...
import { parseStudentUploadFile } from '../utils/bulkStudentUploadUtils'

const BulkStudentUploadModal = ({ isOpen, onClose, onCompleted }) => {
  ...
  const handleUpload = async () => {
    ...
    try {
      const { validStudents, rowErrors } = await parseStudentUploadFile(file)
      ...
      const response = await apiPost('users/bulk-students', {
      ...
      if (!result.success) {
        ...
        showSuccess(result.message || 'Bulk student upload completed.')
      }
  ...
  return (
    <div style={styles.backdrop} role='dialog' aria-modal='true'>
      <div style={styles.modal}>
        <h2 style={styles.title}>Bulk Upload Students</h2>
        <p style={styles.description}>
          Upload a CSV or XLSX file to create multiple student accounts at once.
          The following columns are required:{' '}
          <strong>
            first name, last name, email address, user_id, phone number, school
            year, semester, department, course, section, year level, date of
            birth
          </strong>
          . Guardian and emergency contact details are optional.
        </p>

        <div style={styles.fieldGroup}>
          <label htmlFor='bulkUploadFile' style={styles.label}>
            Student CSV/XLSX File
          </label>
          <input
            id='bulkUploadFile'
            type='file'
        ...
        {summary && (
          <div style={styles.summaryBox}>
            ...
            <p>
              <strong>Created:</strong> {summary.createdCount}
            </p>
            <p>
